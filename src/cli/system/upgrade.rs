...
/// that are already installed: apk/apt/aur/dnf/pacman/zypper upgrade to the
/// newest available version (apk, apt, dnf, and zypper honor a version pinned in
/// config), brew pours the formula's current bottle and replaces the old keg,
/// brew-cask installs the current cask artifact, flatpak and flatpak-user update
